/*
* 做题感受总结：先搞清楚基本概念问题啊，比如【什么叫x的平方根】？搞清楚基本概念后 再找思路和解决方案。不要一上来就找答案，基本概念都没弄懂啊，这会导致舍本求末，事倍功半。
*
* 1.题目需求：
*	输入：x，一个非负整数。
...
*		todo:1.补充出结论a <=根号x <= x/2的推导逻辑；
*			 2.说明这两个语句要这么写的原因：防止整形溢出：
*			mid = left + (right - left)/2、(mid>x/mid)  instead of mid*mid>x
*           (原形：mid = (right + left)/2、mid*mid>x；
*				  如上两种计算原形，如果数值太大，其计算结果会产生溢出的可能：right + left、mid*mid)
