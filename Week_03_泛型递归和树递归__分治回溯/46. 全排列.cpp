...
*
*	3.4.使用编程的方法得到全排列，就是在这样的一个树形结构中完成遍历，
...
*	3.5.执行深度优先遍历，从较深层的结点返回到较浅层结点的时候，需要做「状态重置」，即「深入下一层」、「恢复现场」
*
*	3.6.树的最后一层的所有叶子节点，就是最终的答案集合(全排列的所有情况)。
*	
*	3.7.使用一个数组res来保存 所有可能的全排列结果。
*
* 3.复杂度分析:
...
class Solution {
	public:
		// 递归函数backtrack
		void backtrack(vector<vector<int>>& res, vector<int>& intput, int first, int len){
			//1.递归结束条件：序列nums里的元素都遍历完了（也就是已遍历完某一个叶子节点了，开始回溯） 
			if (first == len) {
				res.emplace_back(intput);  //取得一个全排列结果，存入结果集res
				return;
			}

			// for循环意义： first用于纵向递归找出全排列(叶子节点)，i用于在回溯后横向遍历找全排列；
			for (int i = first; i < len; ++i) {
				
				//2.当前层处理：交换元素位置 
				swap(intput[i], intput[first]);

				//3.深入下一层，直到得出叶子节点(全排列结果)，再回溯
				backtrack(res, intput, first + 1, len);
